...
const Modal = ({ title, description, children, showModal, closeModal, className, allowBackgroundScroll = false }) => {
  ...
  return (
    <AnimatePresence mode="wait">
      {showModal && (
        <motion.div
          className={`fixed inset-0 z-50 flex items-center justify-center overflow-x-hidden overflow-y-hidden md:overflow-y-auto outline-none focus:outline-none bg-opacity-60 backdrop-blur-md`}
          onClick={closeModal}
          initial="hidden"
          animate="visible"
          exit="hidden"
